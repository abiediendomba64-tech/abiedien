...
def init_db():
    ...
    safe_execute("""
        CREATE TABLE IF NOT EXISTS themes (
            ...
            accent_color DEFAULT '#60a5fa',
            bg_color DEFAULT '#0f172a',
            text_color DEFAULT '#f1f5f9',
            landing_template TEXT DEFAULT 'default',
            landing_title TEXT DEFAULT 'Selamat Datang',
            landing_subtitle TEXT DEFAULT '',
            landing_enabled BOOLEAN DEFAULT 1,
    ...
    default_theme = safe_execute("SELECT id FROM themes WHERE name='default'", fetch=True)
    if not default_theme:
        safe_execute("""
            INSERT INTO themes (name, primary_color, secondary_color, accent_color, bg_color, text_color, landing_template, landing_title, is_active)
            VALUES ('default', '#1e40af', '#3b82f6', '#60a5fa', '#0f172a', '#f1f5f9', 'default', 'Selamat Datang', 1)
        """)

    # ================== PANEL BACKOFFICE ACCOUNTS ==================
    safe_execute("""
        CREATE TABLE IF NOT EXISTS panel_accounts (
            ...
            panel_url TEXT DEFAULT '/backoffice',
